...
object Limits {
    ...
    const val WORLD_HEIGHT_BLOCKS = 256

    const val CHUNK_LENGTH_BLOCKS = 16
    const val CHUNK_SIZE_BLOCKS = CHUNK_LENGTH_BLOCKS * CHUNK_LENGTH_BLOCKS * WORLD_HEIGHT_BLOCKS
    ...
    val MIN_BLOCK_POINT = BlockPos(
        x = -20_037_508, // This is the smallest West-facing side coordinate of any block and thus the Western world border.
        y = 0, // This is the lowest bottom side of any block and thus the bottom world border. Using only non-negative integers for heights.
        z = -20_003_931 // This is the smallest North-facing side coordinate of any block and thus the Northern world border.
    )
    val MAX_BLOCK_POINT = BlockPos(
        x = 20_037_508, // This is the largest West-facing side coordinate of any block. The largest East-facing side (and thus the Eastern world border) would be one unit higher.
        y = WORLD_HEIGHT_BLOCKS - 1, // This is the highest bottom side of any block, the highest top side would be one unit higher, which would be the highest point where a person can stand.
        z = 20_003_931 // This is the largest North-facing side coordinate of any block. The South-facing side (and thus the Southern world border) would be one unit higher.
    )
    ...
    private const val DISK_REGION_SIZE_MB_INEXACT = DISK_REGION_SIZE_BYTES / 1024 / 1024
